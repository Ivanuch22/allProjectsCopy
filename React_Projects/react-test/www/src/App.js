...
function App() {
  ...
  const renderCards = (cards, isShowButtons = true) => {
    console.log(isShowButtons)
    return cards.map((card, index) => {
      return <Card obj={card} key={index} index={index} onClick={(nameButton, card) => onClick(nameButton, card)} isShowButtons={isShowButtons} />
    });
  }
        ...
        <div className='content'>
          <Routes>
            ...
            <Route path="/profile" element={
              <ProfilePage setBoughtCards={setBoughtCards} items={boughtCards} isDrawerOpen={isDrawerOpen} priceAddedCards={priceAddedCards} changeDrawerStatus={changeDrawerStatus} addedCarts={addedCarts} input={input} showCartSearched={showCartSearched} searchedCards={searchedCards} renderCards={(addedCarts) => renderCards(addedCarts, false)} onClick={(nameButton, card) => onClick(nameButton, card)} />
            }></Route>
          </Routes >
        </div>
      </div>
