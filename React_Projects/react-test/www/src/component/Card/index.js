...
//фотки
import likee from "./likeCard.svg"
import checkMark from './checkMark.svg'

const Card = ({ obj, index, onClick, isShowButtons }) => {
  ...
  //функція при натиску на кнопку лайк в якій міняємо данні в стайті, обєкті і класи для правильного відображення на екрані
  const like = () => {
    onClick('like', obj, index)
    !obj.isLike ? setLikedClass(style.liked) : setLikedClass('')
  }
  //функція при натиску на кнопку добавлення в кошик в якій міняємо данні в стайті, обєкті і класи для правильного відображення на екрані

  const add = () => {
    onClick('add', obj, index)
    !obj.isAdded ? setAddedClass(style.added) : setAddedClass('')
  }
  ...
  return (
    <>
      <div className={style.card} >
        ...
        <img className={style.img} src={obj.img} alt="фото кросівка" />
        <h2 className={style.title}>{obj.name}</h2>
        <p className={style.description}>Цена: <span className={style.price}>{obj.price} руб.</span></p>

        {isShowButtons ? <div onClick={() => like()} style={obj.isLike ? { background: "#FEF0F0" } : { background: "transparent" }} className={`${style.like} ${style.button}`}>
          {obj.isLike ?
        ...
        {isShowButtons ? <div onClick={() => add()} style={obj.isAdded ? { background: "linear-gradient(180deg, #89F09C 0%, #3CC755 100%)" } : { background: "transparent" }} className={`${style.add} ${style.button}`} >
          {obj.isAdded ?
